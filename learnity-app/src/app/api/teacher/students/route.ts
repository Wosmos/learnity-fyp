  ...
  try {
    ...
    courses.forEach(course => {
        course.enrollments.forEach(enrollment => {
            ...
            if (new Date(enrollment.lastAccessedAt).getTime() > new Date(existingStudent.lastActive).getTime()) {
    ...
    const students = Array.from(studentMap.values()).map(student => {
        ...
        const totalProgress = student.enrolledCourses.reduce((acc: number, course) => acc + course.progress, 0);
        student.totalProgress = Math.round(totalProgress / student.enrolledCourses.length);
        return student;
    });
    ...
    const totalCohortProgress = students.reduce((acc, student) => acc + student.totalProgress, 0);
