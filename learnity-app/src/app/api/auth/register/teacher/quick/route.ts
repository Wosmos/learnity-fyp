  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if ((error as any)?.code === 'P2002' && attempt < maxRetries) {
        console.log(
          `🔄 Retry attempt ${attempt}/${maxRetries} due to unique constraint violation`
        );
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingUser) {
      console.log(
        '🔵 User already exists, checking if teacher profile exists...'
      );
      ...
      if (existingEmail && existingEmail.firebaseUid !== decodedToken.uid) {
        console.error(
          '❌ Email already registered by different user:',
          validatedData.email
        );
        return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      message:
        'Teacher application submitted successfully! Check your email for next steps.',
      data: {
        userId: result.user.id,
    ...
    if ((error as any)?.code === 'P2002') {
      ...
      if (target?.includes('firebaseUid')) {
        return NextResponse.json(
          {
            error:
              'User account already exists. Please try logging in instead.',
          },
          { status: 409 }
        );
