...
async function handleGetUserStats(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const signupGrowthRate = previousSignups > 0 
    ...
    const stats = {
      ...
      // Additional insights
      teacherApprovalRate: pendingTeacherCount + teacherCount > 0 
        ? (teacherCount / (pendingTeacherCount + teacherCount + rejectedTeacherCount)) * 100 
        : 0,
