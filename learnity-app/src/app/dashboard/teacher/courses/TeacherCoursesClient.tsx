...
/**
 * Teacher Courses Client Component
...
export function TeacherCoursesClient({ courses: initialCourses }: Props) {
  const { toast } = useToast();
  const authenticatedFetch = useAuthenticatedFetch();

  const [courses, setCourses] = useState<TeacherCourse[]>(initialCourses);
  ...
  const handlePublish = async (courseId: string) => {
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(
          data.error?.message || data.message || 'Failed to publish course'
        );
      }
  ...
  const handleUnpublish = async (courseId: string) => {
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(
          data.error?.message || data.message || 'Failed to unpublish course'
        );
      }
  ...
  const handleDelete = async (courseId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(
          data.error?.message || data.message || 'Failed to delete course'
        );
      }
