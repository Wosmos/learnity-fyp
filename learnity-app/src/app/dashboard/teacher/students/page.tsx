...
import {
  Users, Search, Calendar, BookOpen,
  Zap, Plus, Filter, BarChart3, GraduationCap, 
  Sparkles, ChevronRight, FileText, Mail,
...
export default function TeacherCommandCenter() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [studentsRes, coursesRes, scheduleRes, statsRes] = await Promise.all([
  ...
  const filteredStudents = useMemo(() => {
    ...
    return studentsData.students.filter(student =>
      ...
      student.enrolledCourses.some(c => c.courseTitle.toLowerCase().includes(query))
    );
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-500 font-medium animate-pulse">Syncing your command center...</p>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-4 max-w-7xl flex justify-between items-center">
          <div className="flex items-center gap-8 text-sm font-medium">
            ...
            <div className="hidden md:flex items-center gap-2 text-emerald-600">
              <Users className="h-4 w-4" /> {studentsData?.activeCount || 0} Students Active
            </div>
          </div>
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 font-medium leading-relaxed max-w-xl">
              Manage your academic ecosystem, track student growth, and refine your curriculum from one dashboard.
            </p>
          </div>
            ...
            {[
              ...
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.id ? "bg-indigo-600 text-white shadow-lg shadow-indigo-200" : "text-slate-500 hover:bg-white hover:text-indigo-600"
                )}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              >
                ...
                {activeTab === 'curriculum' && (
                  <CurriculumView 
                    ...
                    onCreateCourse={() => router.push('/dashboard/teacher/courses/new')}
                  />
                )}
          ...
          <aside className="lg:col-span-4 space-y-8">
              ...
              <CardContent className="p-8 space-y-6 relative z-10">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-slate-400 text-sm">
                    {studentsData && studentsData.students.filter(s => s.totalProgress < 50).length > 0 
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-slate-500 uppercase px-2 tracking-widest">Performance Snapshot</h4>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-white border border-slate-100 shadow-sm">
                  <div className="text-2xl font-black text-indigo-600">
                    {studentsData?.averageProgress || 0}%
                  </div>
                  <div className="text-[10px] uppercase font-bold text-slate-400">Avg Progress</div>
                </div>
                <div className="p-4 rounded-xl bg-white border border-slate-100 shadow-sm">
            ...
            {/* Recent Reports */}
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-slate-500 uppercase px-2 tracking-widest">Quick Actions</h4>
              {[
                { name: 'Export Student Data', action: 'export', icon: Download },
                { name: 'View All Reviews', action: 'reviews', icon: FileText }
              ].map((item, i) => (
                <div 
                  key={i} 
                  onClick={() => toast({ title: 'Coming Soon', description: `${item.name} feature is under development.` })}
            ...
            <Button 
              ...
              onClick={() => window.location.href = 'mailto:support@learnity.com'}
            >
...
function RosterView({ students, searchQuery, onSearchChange, total }: RosterViewProps) {
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium italic">
                    ...
                    {student.totalCoursesEnrolled > 1 && ` +${student.totalCoursesEnrolled - 1} more`}
              ...
              <div className="flex items-center gap-6">
                <div className="text-right">
                  ...
                  <p className="font-bold text-indigo-600">{student.totalProgress}%</p>
                </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {courses.length === 0 ? (
        <Card className="p-8 text-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Create your first course to start teaching</p>
            </div>
        ...
        courses.map((course) => (
          <Card key={course.id} className="overflow-hidden border-none shadow-xl shadow-indigo-100/50 bg-white/80 backdrop-blur group">
            <div className={cn(
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium mt-1">
                      {course.lessonCount} Lessons • {formatDuration(course.totalDuration)} • {course.enrollmentCount} Students
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <Button variant="ghost" className="rounded-xl text-slate-500">Edit</Button>
                  <Button variant="outline" className="rounded-xl border-indigo-100 text-indigo-600">Preview</Button>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="p-8 text-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Schedule live sessions to engage with your students</p>
            </div>
          </div>
        </Card>
      ) : (
        events.map((event) => (
          ...
          >
            <div className="w-16 text-center">
              <p className="text-[10px] font-black text-slate-400 uppercase">{formatEventDay(event.startTime)}</p>
              <p className="text-xl font-black text-slate-900">{formatEventTime(event.startTime)}</p>
            </div>
            <div className="flex-1 flex items-center justify-between">
              ...
              <Button 
                className={cn(
                  ...
                  event.isLive ? "bg-indigo-600" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                )}
              >
