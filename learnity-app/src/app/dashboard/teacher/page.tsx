...
const CourseRow = ({ course }: { course: RecentCourse }) => {
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge
        className={`${styles[status as keyof typeof styles] || ''} border-0`}
      >
        {label}
      </Badge>
    );
  ...
  return (
    <div className='group flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all duration-200'>
      <div className='flex items-center gap-4 min-w-0'>
        <div className='h-10 w-10 rounded-lg bg-slate-50 flex items-center justify-center text-blue-600 font-bold text-xs shrink-0'>
...
export default function TeacherDashboard() {
    ...
    try {
      ...
      if (coursesRes.ok) {
        ...
        const totalStudents = courses.reduce(
          (sum: any, c: any) => sum + (c.enrollmentCount || 0),
          0
        );
        const totalLessons = courses.reduce(
          (sum: any, c: any) => sum + (c.lessonCount || 0),
          0
        );
        ...
        const ratedCourses = courses.filter(
          (c: any) => (c.averageRating || 0) > 0
        );
        const avgRating = ratedCourses.length
          ? ratedCourses.reduce(
              (sum: any, c: any) => sum + Number(c.averageRating || 0),
              0
            ) / ratedCourses.length
          : 0;
        ...
        setStats({
          ...
          totalReviews: courses.reduce(
            (sum: any, c: any) => sum + (c.reviewCount || 0),
            0
          ),
        });
  ...
  useEffect(() => {
    ...
    if (!loading && claims?.role === UserRole.PENDING_TEACHER)
