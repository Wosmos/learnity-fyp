...
import {
  Video, Calendar, Clock, User, CheckCircle, XCircle,
  AlertCircle, Play, X,
} from 'lucide-react';
...
export function StudentSessionsClient({ sessions: initialSessions }: Props) {
  ...
  const handleCancelSession = async (sessionId: string) => {
    ...
    try {
      const response = await authenticatedFetch(`/api/tutoring-sessions/${sessionId}/cancel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reason: 'Cancelled by student' }),
      });
      if (response.ok) {
        toast({ title: 'Session Cancelled', description: 'The tutoring session has been cancelled.' });
  ...
  const handleJoinSession = async (sessionId: string) => {
    try {
      const response = await authenticatedFetch(`/api/tutoring-sessions/${sessionId}/join`);
      if (response.ok) {
        toast({ title: 'Joining Session', description: 'Opening video conference...' });
      } else {
  ...
  const filteredSessions = sessions.filter(session => {
    if (activeTab === 'upcoming') return ['PENDING', 'ACCEPTED', 'SCHEDULED'].includes(session.status);
    if (activeTab === 'completed') return session.status === 'COMPLETED';
    if (activeTab === 'cancelled') return ['REJECTED', 'CANCELLED'].includes(session.status);
    return true;
  });

  return (
    <div className='min-h-screen bg-background'>
      <PageHeader title='My Tutoring Sessions' subtitle='Manage your one-on-one learning sessions' icon={Video} />
            ...
            {filteredSessions.length > 0 ? (
                    ...
                    <CardContent className='p-6'>
                      <div className='flex flex-col md:flex-row gap-6'>
                        <div className='flex items-start gap-4 flex-1'>
                          <div className='w-16 h-16 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-xl flex-shrink-0'>
                            {session.teacher.profilePicture ? (
                              <img src={session.teacher.profilePicture} alt={`${session.teacher.firstName} ${session.teacher.lastName}`} className='w-full h-full rounded-full object-cover' />
                            ) : (
                          ...
                          <div className='flex-1 min-w-0'>
                            ...
                            <div className='flex flex-wrap gap-4 text-sm text-muted-foreground'>
                              ...
                              <div className='flex items-center gap-1'>
                                <Video className='h-4 w-4' />{session.duration} minutes
                              </div>
                            </div>
                            {session.rejectionReason && (
                              <div className='mt-3 p-3 bg-red-50 border border-red-200 rounded-lg'>
                                <p className='text-sm text-red-700 flex items-start gap-2'>
                                  ...
                                  <span><strong>Rejected:</strong> {session.rejectionReason}</span>
                                </p>
                              </div>
                            )}
                            {session.cancellationReason && (
                              <div className='mt-3 p-3 bg-muted border border-border rounded-lg'>
                                <p className='text-sm text-muted-foreground'><strong>Cancelled:</strong> {session.cancellationReason}</p>
                              </div>
                            )}
                          </div>
                        </div>
                        <div className='flex md:flex-col gap-2 md:w-32'>
                          {session.status === 'LIVE' && (
                            <Button onClick={() => handleJoinSession(session.id)} className='flex-1 md:flex-none bg-green-600 hover:bg-green-700'>
                              <Play className='h-4 w-4 mr-2' />Join
                            </Button>
                          )}
                          {['PENDING', 'ACCEPTED', 'SCHEDULED'].includes(session.status) && (
                            <Button onClick={() => handleCancelSession(session.id)} variant='outline' className='flex-1 md:flex-none text-red-600 hover:text-red-700 hover:bg-red-50'>
              ...
              <Card>
                <CardContent className='p-12 text-center'>
                  ...
                  <Button onClick={() => router.push('/teachers')} className='bg-indigo-600 hover:bg-indigo-700'>Browse Teachers</Button>
                </CardContent>
              </Card>
            )}
          </TabsContent>
