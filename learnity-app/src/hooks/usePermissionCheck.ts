 ...
 */
export function usePermissionCheck(permissions: Permission[]): PermissionCheckResult {
 ...
 */
export function useAnyPermission(permissions: Permission[]): PermissionCheckResult {
  const { claims, loading: authLoading } = useAuth();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
...
export function useAdminPermissions() {
  ...
  return {
    ...
    loading: canAccessAdmin.loading || canManageUsers.loading || canApproveTeachers.loading || canViewAuditLogs.loading || canManagePlatform.loading,
    error: canAccessAdmin.error || canManageUsers.error || canApproveTeachers.error || canViewAuditLogs.error || canManagePlatform.error
  };
}
...
export function useTeacherPermissions() {
  const canAccessTeacherDashboard = usePermissionCheck([Permission.VIEW_TEACHER_DASHBOARD]);
  ...
  const canViewStudentProgress = usePermissionCheck([Permission.VIEW_STUDENT_PROGRESS]);
  ...
  return {
    ...
    loading: canAccessTeacherDashboard.loading || canManageSessions.loading || canUploadContent.loading || canViewStudentProgress.loading,
    error: canAccessTeacherDashboard.error || canManageSessions.error || canUploadContent.error || canViewStudentProgress.error
  };
}
...
export function useStudentPermissions() {
  const canAccessStudentDashboard = usePermissionCheck([Permission.VIEW_STUDENT_DASHBOARD]);
  ...
  return {
    ...
    loading: canAccessStudentDashboard.loading || canJoinStudyGroups.loading || canBookTutoring.loading || canEnhanceProfile.loading,
    error: canAccessStudentDashboard.error || canJoinStudyGroups.error || canBookTutoring.error || canEnhanceProfile.error
  };
}
...
export function usePendingTeacherPermissions() {
  const canViewApplicationStatus = usePermissionCheck([Permission.VIEW_APPLICATION_STATUS]);
  const canUpdateApplication = usePermissionCheck([Permission.UPDATE_APPLICATION]);
...
export function useDynamicPermissionCheck() {
  ...
  const checkPermission = useCallback(async (permission: Permission): Promise<boolean> => {
    ...
    try {
      const hasPermission = await roleManager.hasPermission(user.uid, permission);
...
export function useProfileRequirements() {
  ...
  const requirements = useMemo(() => {
    ...
    return {
      ...
      canAccessPlatform: claims.emailVerified && claims.profileComplete && Object.values(UserRole).includes(claims.role)
    };
  }, [claims]);
