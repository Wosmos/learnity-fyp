...
export function useAuthenticatedFetch() {
  ...
  const authenticatedFetch = useCallback(
    async (
      ...
      if (!skipAuth && !user) {
        console.error(
          '[useAuthenticatedFetch] No authenticated user found for',
          url
        );
      ...
      if (!skipAuth && user) {
        try {
          ...
          console.error(
            '[useAuthenticatedFetch] Failed to get ID token:',
            error
          );
          ...
          try {
            ...
            console.error(
              '[useAuthenticatedFetch] Failed to get ID token on retry:',
              retryError
            );
            throw new Error(
              'Authentication failed - unable to get valid token'
            );
          }
        }
      }
      ...
      if (
        ...
        typeof finalHeaders === 'object' &&
        !Array.isArray(finalHeaders)
      ) {
      ...
      if (response.status === 401) {
        ...
        if (!skipAuth && user) {
          try {
            ...
            console.error(
              '[useAuthenticatedFetch] Token refresh failed:',
              error
            );
          }
        }
