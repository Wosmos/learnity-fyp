  ...
  "relative w-full rounded-[2rem] border p-5 backdrop-blur-xl transition-all duration-500 [&>svg~*]:pl-9 [&>svg+div]:translate-y-[-1px] [&>svg]:absolute [&>svg]:left-5 [&>svg]:top-5 [&>svg]:size-5",
  {
    variants: {
      variant: {
        ...
        default: 
          "bg-white/[0.03] text-slate-300 border-white/10 shadow-2xl",
...
>(({ className, ...props }, ref) => (
  <h5
    ref={ref}
    className={cn("mb-1.5 font-black uppercase italic tracking-widest text-sm leading-none", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("text-xs font-medium text-slate-500 leading-relaxed", className)}
    {...props}
  />
))
