    ...
    >
      ...
      {showLogo && (
        <div className='flex items-center space-x-3 mb-6'>
          <div
            className={cn(
              'bg-gradient-to-br from-blue-600 to-purple-600 rounded-xl shadow-lg flex items-center justify-center',
              sizes.logoContainer
            )}
          >
      ...
      <div className='flex items-center space-x-3 mb-4'>
        <Loader2
          className={cn('animate-spin text-blue-600', sizes.spinner)}
          aria-hidden='true'
        />
      ...
      <span className='sr-only'>
        Please wait while we verify your authentication status. This should only
        take a moment.
      </span>
    </div>
  );
...
export function AuthLoadingOverlay({
  ...
  return (
    <div
      ...
      aria-label={message}
    >
      <AuthLoadingSpinner message={message} showLogo={true} size='lg' />
    </div>
  );
}
