...
export function ProfileEnhancementForm({ initialData, onSuccess }: ProfileEnhancementFormProps) {
  ...
  const [gradeLevel, setGradeLevel] = useState(initialData?.gradeLevel || 'Not specified');
  ...
  const gradeOptions = [
    'Middle School', 'High School', 'Undergraduate', 'Graduate', 'Professional', 'Other'
  ];
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <section className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm transition-all hover:shadow-md">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Update your name and academic level</p>
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {gradeOptions.map(opt => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${gradeLevel === opt
          ...
          <section className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm transition-all hover:shadow-md">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Tell us a bit about your academic journey</p>
              </div>
            </div>
            <Textarea
              value={bio}
          ...
          <section className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm transition-all hover:shadow-md">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-800">Subjects I Study</h3>
                <p className="text-xs text-slate-500">Pick the areas you're currently focused on</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <section className="bg-slate-900 p-8 rounded-[32px] text-white shadow-xl shadow-slate-200 group relative overflow-hidden">
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-10">
                <div>
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Profile Complete</h4>
                  <div className="text-5xl font-black italic tracking-tighter text-indigo-400">{completion?.percentage || 0}%</div>
                </div>
          ...
          <section className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-800">Learning Goals</h3>
                <p className="text-xs text-slate-500">What are you aiming to achieve?</p>
              </div>
            </div>
            <div className="space-y-4">
              <div className="flex gap-2">
                ...
                />
                <Button size="icon" onClick={() => handleAddItem(newGoal, setNewGoal, learningGoals, setLearningGoals)} className="h-10 w-10 bg-slate-900 rounded-xl shrink-0">
              ...
              <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1 custom-scrollbar">
                <AnimatePresence>
                  {learningGoals.map(goal => (
                    <motion.div initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} key={goal} className="flex items-center justify-between p-3 bg-slate-50/80 rounded-xl group/item">
                      <span className="text-[11px] font-medium text-slate-600">{goal}</span>
                      <X className="w-3.5 h-3.5 text-slate-300 cursor-pointer hover:text-red-500 transition-colors" onClick={() => setLearningGoals(learningGoals.filter(g => g !== goal))} />
                    </motion.div>
                  ))}
                </AnimatePresence>
              </div>
          ...
          <section className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-800">Study Style</h3>
              ...
              {studyPreferenceOptions.map(pref => {
                ...
                return (
                  <button
                    ...
                    className={`text-[10px] font-bold py-2.5 px-3 rounded-xl border transition-all ${active
          ...
          <section className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">What do you enjoy outside study?</p>
              </div>
            </div>
            <div className="space-y-4">
              <div className="flex gap-2">
                <Input
                  ...
                  className="rounded-xl border-slate-100 bg-slate-50/50 text-sm"
                />
                <Button size="icon" onClick={() => handleAddItem(newInterest, setNewInterest, interests, setInterests)} className="h-10 w-10 bg-slate-900 rounded-xl shrink-0">
                  <Plus className="w-4 h-4" />
                </Button>
              </div>
              <div className="flex flex-wrap gap-2">
                {interests.map(interest => (
                  <Badge key={interest} variant="secondary" className="px-3 py-1.5 bg-slate-100 text-slate-600 border-none rounded-lg text-[11px]">
                    {interest}
                    <X className="w-3 h-3 ml-2 cursor-pointer hover:text-red-500" onClick={() => setInterests(interests.filter(i => i !== interest))} />
                  </Badge>
                ))}
              </div>
            </div>
