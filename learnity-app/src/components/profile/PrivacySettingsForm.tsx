...
export function PrivacySettingsForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: 'System Updated', description: 'Privacy protocols successfully applied.' });
      onSuccess?.();
    } catch (error: any) {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-100 pb-8 gap-4">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 px-2 py-1 rounded-md bg-indigo-50 border border-indigo-100 mb-2">
            ...
            <span className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest">Secure Configuration</span>
          </div>
          ...
          <p className="text-sm text-slate-500 max-w-md leading-relaxed">
            Configure your digital presence. Higher visibility increases collaboration, while private modes prioritize focus.
          </p>
        </div>
      </div>
            ...
            >
              ...
              <SelectContent className="rounded-xl border-slate-100 shadow-2xl">
                <SelectItem value="PUBLIC" className="py-3 focus:bg-indigo-50">
                  <span className="flex items-center gap-2 font-bold"><Globe className="w-4 h-4 text-emerald-500" /> Open Broadcast</span>
                </SelectItem>
                <SelectItem value="FRIENDS" className="py-3 focus:bg-indigo-50">
                  <span className="flex items-center gap-2 font-bold"><Users className="w-4 h-4 text-blue-500" /> Circle Only</span>
                </SelectItem>
                <SelectItem value="PRIVATE" className="py-3 focus:bg-indigo-50">
                  <span className="flex items-center gap-2 font-bold"><Lock className="w-4 h-4 text-slate-400" /> Stealth Mode</span>
                </SelectItem>
              </SelectContent>
            </Select>
      ...
      <div className="sticky bottom-4 z-20 pt-8 mt-12">
        <div className="bg-white/70 backdrop-blur-xl border border-slate-200/50 p-4 rounded-3xl shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 px-4">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Preferences Encrypted</span>
          </div>
          <LoadingButton
...
function SectionLabel({ icon: Icon, label }: { icon: any, label: string }) {
  return (
    <div className="flex items-center gap-2 px-1">
      ...
      <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">{label}</h3>
    </div>
  );
}
...
function VisibilityHelper({ status }: { status: string }) {
  const configs: Record<string, { color: string, text: string, icon: any }> = {
    PUBLIC: { color: 'text-emerald-600', text: 'Identity is discoverable by all platform nodes.', icon: Globe },
    FRIENDS: { color: 'text-blue-600', text: 'Visibility limited to confirmed network connections.', icon: Users },
    PRIVATE: { color: 'text-slate-600', text: 'Identity encrypted. Hidden from discovery protocols.', icon: Lock },
  };
  ...
  return (
    <div className="flex gap-3">
      ...
      <p className={cn("text-xs leading-relaxed font-bold", config.color)}>{config.text}</p>
    </div>
  );
}
    ...
    >
      <div className="flex items-center gap-4">
        <div className={cn(
          ...
          isOn ? "bg-slate-900 text-white" : "bg-slate-200 text-slate-500 group-hover:bg-slate-300"
        )}>
