...
import { Trash2, Camera, Loader2, User, Plus, Info, ShieldCheck } from 'lucide-react';
...
export function AvatarUpload({ 
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await authenticatedFetch('/api/profile/avatar', { method: 'POST', body: formData });
      if (!response.ok) throw new Error('Upload failed');
      const data = await response.json();
      setPreview(data.avatarUrl);
      toast({ title: 'Identity Synced', description: 'Visual vector updated.' });
  ...
  const handleDelete = async () => {
    if (authLoading || !confirm('Permanently remove this identity visual?')) return;
    setDeleting(true);
    try {
    ...
    <TooltipProvider delayDuration={100}>
      <div className="flex flex-col items-center gap-6 p-2">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative">
            <div className={cn(
              ...
              >
                ...
                <span className="text-[8px] font-black uppercase text-white tracking-[0.2em] mt-1">Sync</span>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 py-1">
            <Tooltip>
              ...
              <TooltipContent side="right" className="bg-black text-white text-[10px] font-bold uppercase tracking-widest border-none">
                Update Visual
              </TooltipContent>
            </Tooltip>
            ...
            {preview && (
              <Tooltip>
                ...
                <TooltipContent side="right" className="bg-red-500 text-white text-[10px] font-bold uppercase tracking-widest border-none">
                  Purge Data
                </TooltipContent>
              </Tooltip>
            )}
            ...
            <Tooltip>
              ...
              <TooltipContent side="right" className="max-w-[200px] p-3 space-y-2 bg-white border border-slate-100 shadow-xl text-slate-600">
                <p className="text-[10px] font-bold uppercase tracking-tight text-black">Upload Protocols:</p>
                <ul className="text-[9px] space-y-1 list-disc pl-3 leading-relaxed">
                  <li>Formats: <span className="font-bold">JPEG, PNG, WEBP</span></li>
                  <li>Max Payload: <span className="font-bold">5.0 MB</span></li>
                  <li>Ratio: <span className="font-bold">1:1 Square</span> recommended</li>
        ...
        <div className="flex flex-col items-center gap-1">
          <span className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-400">Identity Visualizer</span>
