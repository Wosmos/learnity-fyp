      ...
      <div className={cn("relative", item.className)}>
        ...
        {typeof item.content === 'string' ? (
          <div className="space-y-6">
            {item.content.split('\n\n').map((paragraph, index) => (
              <p key={index} className="text-lg text-slate-500 font-medium leading-relaxed italic border-l-2 border-slate-100 pl-6">
                {paragraph}
              </p>
            ))}
          </div>
      ...
      <div className={cn(
        ...
        {Icon && (
          <div className={cn(
            ...
            item.gradient ? "bg-white/10 text-indigo-400" : "bg-slate-950 text-white"
          )}>
