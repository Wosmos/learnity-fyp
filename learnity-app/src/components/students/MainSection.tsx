...
import { 
  ...
  Clock, Zap, Play, Code2, Sigma, Beaker, GraduationCap
} from 'lucide-react';
...
const MobileCourseCard = ({ enrollment, onClick }: { enrollment: CourseEnrollment; onClick: () => void }) => {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-2 text-xs text-slate-400 font-medium">
          ...
          <span>{Math.floor(enrollment.course.totalDuration / 60)}h {enrollment.course.totalDuration % 60}m</span>
  ...
  if (error) {
    return (
      <div className="lg:col-span-2 bg-white rounded-2xl border border-red-100 p-8 text-center">
        ...
        <Button variant="outline" size="sm" onClick={fetchEnrollments} className="text-xs">
          Try Again
        </Button>
      </div>
    );
        ...
        <div className="space-y-3">
          {isLoading ? (
            ...
            <div className="text-center py-8 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
               ...
               <Link href="/dashboard/student/public-cources" className="text-xs text-indigo-600 font-bold mt-2 inline-block">Browse Catalog</Link>
            </div>
          ) : (
            ...
            <>
             ...
             <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center">
              ...
              <Link href="/dashboard/student/public-cources">
                <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs">Browse Courses</Button>
              </Link>
            </div>
          ) : (
            activeCourses.map((enrollment) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                   <h4 className="text-sm font-black text-slate-900 truncate pr-2">{session.title}</h4>
                   {session.status === 'Live' && (
                ...
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tight flex items-center gap-1.5">
                    ...
                    <span className="hidden md:inline-block font-medium normal-case text-slate-500">{session.host}</span>
                </p>
              </div>
              
              <Button size="icon" variant="ghost" className="rounded-full hover:bg-indigo-50 text-slate-300 hover:text-indigo-600 -mr-2">
