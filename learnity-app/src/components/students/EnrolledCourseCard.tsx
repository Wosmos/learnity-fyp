...
export function EnrolledCourseCard({ enrollment, onContinue }: EnrolledCourseCardProps) {
  const { course, progress, status, lastAccessedAt } = enrollment;
  const isCompleted = status === "COMPLETED";

  // Dynamic color selection based on progress or category
  const themeColor = isCompleted ? "from-emerald-500 to-teal-600" : "from-indigo-600 to-violet-700";
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
              <div>
                ...
                <p className="text-sm font-bold text-slate-400 mt-1">
                  with <span className="text-slate-600">{course.teacher.name}</span>
                </p>
              </div>
            ...
            <div className="space-y-5">
              <div className="relative h-2.5 w-full bg-slate-50 rounded-full overflow-hidden border border-slate-100 shadow-inner">
                <motion.div
                  ...
                  className={cn("absolute inset-y-0 left-0 rounded-full shadow-[0_0_12px_rgba(0,0,0,0.1)]", barColor)}
                />
              </div>
