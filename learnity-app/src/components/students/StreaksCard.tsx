                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="flex items-baseline gap-1">
                                <span className="text-2xl font-black text-white tracking-tighter">7</span>
                                <span className="text-[10px] font-bold text-orange-500 uppercase tracking-widest">Days</span>
                            </div>
                            <p className="text-[10px] text-slate-500 font-medium -mt-1">Top 5% Consistency</p>
                        </div>
                    </div>
                            ...
                            <div key={i} className="flex flex-col items-center gap-1">
                                <motion.div
                                    ...
                                    className={cn(
                                        ...
                                        i === 6 ? "h-6 shadow-[0_0_8px_rgba(249,115,22,0.6)]" : "h-4", 
                                        getLevelColor(day.level)
                                    )}
                                />
                                <span className="text-[8px] font-bold text-slate-700 uppercase">{days[i]}</span>
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        <div className="flex gap-[3px] flex-1">
                            ...
                            {Array.from({ length: 5 }).map((_, weekIndex) => (
                                <div key={weekIndex} className="grid grid-rows-7 gap-[3px]">
                                    {desktopActivity.slice(weekIndex * 7, (weekIndex + 1) * 7).map((day, i) => (
                                        <motion.div
                                            key={i}
                            ...
                            <div className="ml-4 self-center space-y-3 flex-1">
                                <div>
                                    <div className="flex justify-between items-end mb-1.5">
                                        <span className="text-[9px] font-bold text-slate-500 uppercase">Next Goal</span>
                                        <span className="text-[9px] font-bold text-white">10 Days</span>
                                    </div>
...
function getLevelColor(level: number) {
    switch (level) {
        ...
        case 1: return "bg-orange-900/40";
        case 2: return "bg-orange-700/60";
        case 3: return "bg-orange-600";
        case 4: return "bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.4)]";
