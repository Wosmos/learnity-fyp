      ...
      <div
        ...
        className={cn(
          ...
          isFullscreen
            ...
            : 'rounded-3xl h-[650px] border border-slate-800',
          className
        )}
      >
        ...
        <div className='absolute top-0 left-0 right-0 z-20 p-6 bg-gradient-to-b from-black/80 to-transparent transition-opacity duration-300 hover:opacity-100 opacity-0 group-hover:opacity-100 flex justify-between items-start pointer-events-none hover:pointer-events-auto'>
          ...
          <div className='flex items-center gap-3 pointer-events-auto'>
            <Badge
              variant='outline'
              className='bg-black/40 border-slate-700 text-slate-300 backdrop-blur-md gap-2 h-9 px-3 rounded-full'
            >
          ...
          <div className='flex-1 p-4 md:p-6 flex items-center justify-center transition-all duration-300'>
            <div
              className={cn(
                ...
                peers.length === 1
                  ? 'grid-cols-1 max-w-4xl' // Spotlight view for 1
                  : peers.length === 2
            ...
            <ScrollArea className='flex-1 p-4'>
              <div className='space-y-6'>
                ...
                {messages.map(msg => (
                  <div
                    key={msg.id}
                    className='flex flex-col gap-1.5 animate-in slide-in-from-bottom-2 duration-300'
                  >
            ...
            <div className='p-4 border-t border-slate-800 bg-white/5'>
              <div className='flex gap-2 items-end'>
                ...
                <Button
                  size='icon'
                  onClick={sendMessage}
                  className='h-11 w-11 bg-indigo-600 hover:bg-indigo-500 rounded-xl shrink-0'
                >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant='ghost'
                  size='icon'
                  className='h-12 w-12 rounded-xl hover:bg-indigo-600/20 hover:text-indigo-400 text-slate-400 transition-colors'
                >
                  <MonitorUp className='h-5 w-5' />
                </Button>
              </TooltipTrigger>
              <TooltipContent className='bg-slate-800 text-white border-slate-700'>
                <p>Share Screen (Coming Soon)</p>
              </TooltipContent>
            </Tooltip>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant='ghost'
                  size='icon'
                  className='h-12 w-12 rounded-xl hover:bg-slate-800 text-slate-300'
                >
              ...
              <DropdownMenuContent
                side='top'
                className='bg-slate-900/95 backdrop-blur-xl border-slate-800 text-slate-300 mb-4 w-56 rounded-xl shadow-2xl p-2'
              >
                <DropdownMenuLabel className='px-2 py-1.5 text-xs font-bold text-slate-500 uppercase tracking-widest'>
                  Settings
                </DropdownMenuLabel>
                <DropdownMenuSeparator className='bg-slate-800' />
                <DropdownMenuItem
                  className='focus:bg-indigo-600 focus:text-white rounded-lg cursor-pointer px-3 py-2.5'
                  onClick={toggleFullscreen}
                >
                  {isFullscreen ? (
