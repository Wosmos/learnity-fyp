  ...
  'inline-flex items-center gap-1.5 font-medium rounded-full transition-all',
  {
    variants: {
      ...
      variant: {
        ...
        gradient:
          'bg-gradient-to-r from-yellow-400/20 to-orange-400/20 text-orange-600 dark:text-orange-400',
...
export function XPBadge({
  ...
  return (
    ...
    >
      {showIcon && (
        <Sparkles className={cn(iconSizes[size ?? 'md'], 'text-yellow-500')} />
      )}
...
export function XPProgress({
  ...
  const progress =
    xpToNextLevel > 0 ? (xpInCurrentLevel / xpToNextLevel) * 100 : 100;
...
function getLevelColor(level: number): string {
  ...
  if (level >= 4)
    return 'bg-slate-100 text-blue-700 dark:bg-slate-900/30 dark:text-blue-400';
