...
export function SectionManager() {
  const { sections, addSection, updateSection, deleteSection, reorderSections } = useCourseBuilder();
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [editingSection, setEditingSection] = useState<{ index: number; data: SectionFormData } | null>(null);
  ...
  const handleDeleteSection = (index: number) => {
    if (confirm('Are you sure you want to delete this section? All lessons will be removed.')) {
      deleteSection(index);
    }
  };
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-slate-500 mb-4 text-center max-w-sm">
              Organize your course content into sections. Each section can contain multiple video lessons and quizzes.
            </p>
