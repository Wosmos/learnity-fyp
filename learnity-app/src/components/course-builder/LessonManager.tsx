...
export function LessonManager({ sectionIndex }: LessonManagerProps) {
  const { sections, addLesson, updateLesson, deleteLesson, reorderLessons } = useCourseBuilder();
  ...
  const validateYouTubeUrl = async (url: string) => {
    ...
    try {
      ...
      if (!lessonTitle && metadata.title) {
        setLessonTitle(metadata.title);
      }
    } catch (error) {
      setUrlError(error instanceof Error ? error.message : 'Invalid YouTube URL');
      setYoutubeMetadata(null);
    } finally {
  ...
  const handleUrlChange = (url: string) => {
    setYoutubeUrl(url);
    // Debounce validation
    const timeoutId = setTimeout(() => validateYouTubeUrl(url), 500);
            ...
            >
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <Button
                  ...
                  size="sm"
                  className="h-7 w-7 p-0"
                  onClick={() => setEditingLesson({ index, data: { ...lesson } })}
                >
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={lessonType} onValueChange={(v) => setLessonType(v as 'VIDEO' | 'QUIZ')}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <div className="space-y-2">
                ...
                {youtubeMetadata && (
                  <div className="p-3 bg-slate-50 rounded-lg border">
                    <p className="text-sm font-medium text-slate-700">{youtubeMetadata.title}</p>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setEditingLesson({
                      ...editingLesson,
                      data: { ...editingLesson.data, description: e.target.value },
                    })
                  }
                  rows={2}
                />
