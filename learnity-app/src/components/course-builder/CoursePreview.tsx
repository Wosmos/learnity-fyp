...
export function CoursePreview() {
  ...
  const isPreviewReady =
    courseData.title && courseData.description && sections.length > 0;
    ...
    return (
      <div className='text-center py-12'>
        ...
        <p className='text-sm text-slate-500 max-w-md mx-auto'>
          Complete the basic information and add at least one section with
          lessons to preview your course.
        </p>
      </div>
    );
  }
    ...
    <div className='space-y-6'>
        ...
        <CardContent>
          <Accordion type='multiple' className='space-y-2'>
            {sections.map((section, sectionIndex) => {
              ...
              const videoCount = section.lessons.filter(
                l => l.type === 'VIDEO'
              ).length;
              const quizCount = section.lessons.filter(
                l => l.type === 'QUIZ'
              ).length;
      ...
      <div className='flex justify-center'>
        <Button size='lg' className='gap-2' disabled>
          ...
          {courseData.isFree
            ? 'Enroll for Free'
            : `Enroll - $${courseData.price?.toFixed(2)}`}
        </Button>
      </div>
    </div>
  );
