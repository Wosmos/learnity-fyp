...
const useActiveRoute = (pathname: string, role: string, navItems: NavItem[]) => {
  return (href: string) => {
    ...
    <div className={cn(
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          <div className={cn(
            "p-2 rounded-xl flex-shrink-0 transition-transform duration-300",
          ...
          <div className={cn(
            ...
            <span className="font-bold text-sm tracking-tight">{config.brandName}</span>
            <span className={cn(
      ...
      <div className={cn(
        ...
        {!isCollapsed && config.upgradePromo && (
          <div className={cn(
            ...
            <h4 className="text-xs font-bold mb-1">{config.upgradePromo.title}</h4>
            <p className="text-[10px] text-slate-500 mb-2 leading-relaxed">
              {config.upgradePromo.description}
            </p>
            <Link href={config.upgradePromo.link} className="text-[10px] font-bold text-blue-500 hover:text-blue-600 flex items-center gap-1">
        ...
        <Link href={`/dashboard/${config.role}/settings`} className="block">
          <Button variant="ghost" className={cn(
            ...
            isDark ? "hover:bg-slate-800 text-slate-400" : "hover:bg-slate-200/50 text-slate-600"
          )}>
          ...
          {mainItems.map((item: NavItem, index: number) => {
              ...
              <Link key={item.href} href={item.href} className="mx-1">
                <motion.div
                  ...
                  className={cn(
                    ...
                    active ? "bg-white text-slate-950 shadow-[0_0_20px_rgba(255,255,255,0.2)]" : "bg-slate-900 text-slate-500"
                  )}
                >
            ...
            return (
              <Link key={item.href} href={item.href} className="relative w-12 h-12 flex items-center justify-center">
                ...
                {active && (
                  <motion.div layoutId="onyxPill" className="absolute inset-1 bg-white/10 rounded-2xl" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
                )}
              </Link>
            );
          })}
...
export function DashboardSidebar({
  ...
  return (
    <>
      <MobileBottomNav config={config} pathname={pathname} onLogout={handleLogout} />
