      ...
      <div className="h-full px-4 md:px-6 mx-auto flex items-center justify-between gap-4 max-w-[1600px]">
            ...
            {stats.xpPoints !== undefined && (
              ...
              >
                ...
                <span className="text-sm font-bold bg-gradient-to-r from-slate-700 to-slate-900 group-hover:from-indigo-600 group-hover:to-purple-600 bg-clip-text text-transparent transition-all">
                  {stats.xpPoints.toLocaleString()} <span className="text-xs font-medium text-slate-400 group-hover:text-indigo-400">XP</span>
                </span>
              </div>
            )}
            ...
            {stats.streak !== undefined && stats.streak > 0 && (
              ...
              >
                ...
                <div className="flex flex-col items-start leading-none -space-y-0.5">
                  <span className="text-xs font-bold text-orange-700">{stats.streak} Day</span>
                  <span className="text-[10px] font-medium text-orange-600/80 uppercase tracking-wide">Streak</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-1.5 md:gap-3 justify-end flex-1 md:flex-none">
          ...
          <div className="relative">
            <Button variant="ghost" size="icon" className="relative h-10 w-10 text-slate-500 hover:text-slate-700 hover:bg-slate-100/80 rounded-full transition-all duration-200">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="pl-1 pr-2 h-10 gap-2 hover:bg-slate-100/50 rounded-full transition-all duration-200 group">
                <div className="relative">
                ...
                <div className="hidden sm:flex flex-col items-start text-left">
                  <span className="text-sm font-semibold text-slate-700 group-hover:text-slate-900 transition-colors">
                    {profileData?.firstName || user?.displayName?.split(' ')[0] || 'User'}
                  </span>
                  <span className="text-[10px] text-slate-400 font-medium leading-none mt-0.5 capitalize">{role}</span>
                </div>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56 mt-2 rounded-xl border-slate-100 shadow-lg shadow-slate-200/50 p-2">
              ...
              <DropdownMenuItem asChild className="cursor-pointer rounded-lg focus:bg-slate-50">
                <Link href={role === 'teacher' ? '/dashboard/teacher/profile/enhance' : '/profile/enhance'}>
