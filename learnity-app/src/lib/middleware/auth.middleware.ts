...
export async function authMiddleware(
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return createErrorResponse(AuthErrorCode.TOKEN_INVALID, 'Missing or invalid authorization header');
    }
    ...
    if (!options.skipEmailVerification && !decodedToken.email_verified) {
      return createErrorResponse(AuthErrorCode.EMAIL_NOT_VERIFIED, 'Email verification required');
    }

    // Check role requirements
    if (options.requiredRole && claims.role !== options.requiredRole) {
      // Check if user has one of the allowed roles
      if (options.allowMultipleRoles && !options.allowMultipleRoles.includes(claims.role)) {
        return createErrorResponse(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'Insufficient role permissions');
    ...
    if (options.requiredPermissions && options.requiredPermissions.length > 0) {
      ...
      if (!hasAllPermissions) {
        return createErrorResponse(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'Insufficient permissions');
      }
    }
    ...
    if (error.code === 'auth/id-token-expired') {
      return createErrorResponse(AuthErrorCode.TOKEN_EXPIRED, 'Token has expired');
    }
    
    if (error.code === 'auth/id-token-revoked') {
      return createErrorResponse(AuthErrorCode.TOKEN_REVOKED, 'Token has been revoked');
    }

    return createErrorResponse(AuthErrorCode.TOKEN_INVALID, 'Invalid authentication token');
  }
}
 ...
 */
export function requireRole(role: UserRole, options: Omit<AuthMiddlewareOptions, 'requiredRole'> = {}) {
  return (request: NextRequest) => authMiddleware(request, { ...options, requiredRole: role });
}

/**
 * Middleware factory for specific permission requirements
 */
export function requirePermissions(permissions: Permission[], options: Omit<AuthMiddlewareOptions, 'requiredPermissions'> = {}) {
  return (request: NextRequest) => authMiddleware(request, { ...options, requiredPermissions: permissions });
}

/**
 * Middleware factory for admin-only routes
 */
export function requireAdmin(options: Omit<AuthMiddlewareOptions, 'requiredRole'> = {}) {
 ...
 */
export function requireTeacher(options: Omit<AuthMiddlewareOptions, 'requiredRole' | 'allowMultipleRoles'> = {}) {
 ...
 */
export function requireStudent(options: Omit<AuthMiddlewareOptions, 'requiredRole'> = {}) {
 ...
 */
export async function routeProtectionMiddleware(request: NextRequest): Promise<NextResponse | null> {
  ...
  const protectedRoutes: Record<string, AuthMiddlewareOptions> = {
    ...
    '/dashboard/teacher': { requiredPermissions: [Permission.VIEW_TEACHER_DASHBOARD] },
    '/dashboard/student': { requiredPermissions: [Permission.VIEW_STUDENT_DASHBOARD] },
    
    // Profile routes
    '/profile/enhance': { requiredPermissions: [Permission.ENHANCE_PROFILE] },
    
    // Application routes
    '/application/status': { requiredPermissions: [Permission.VIEW_APPLICATION_STATUS] },
    '/application/update': { requiredPermissions: [Permission.UPDATE_APPLICATION] }
  };
 ...
 */
function findMatchingRoute(pathname: string, routes: Record<string, AuthMiddlewareOptions>): AuthMiddlewareOptions | null {
 ...
 */
function createErrorResponse(code: AuthErrorCode, message: string): NextResponse {
...
export function withTeacherAuth(
  ...
  return withAuth(handler, { allowMultipleRoles: [UserRole.TEACHER, UserRole.ADMIN] });
}
