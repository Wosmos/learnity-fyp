...
describe('SessionManagerService', () => {
  ...
  describe('Token Pair Generation', () => {
    ...
    it('should generate tokens with correct expiration times', async () => {
      ...
      const accessTokenDuration = tokenPair.accessTokenExpiresAt.getTime() - beforeGeneration.getTime();
      ...
      const refreshTokenDuration = tokenPair.refreshTokenExpiresAt.getTime() - beforeGeneration.getTime();
  ...
  describe('Token Validation', () => {
    it('should validate valid access token', async () => {
      const tokenPair = await sessionManager.generateTokenPair(mockSessionData);
      const validation = await sessionManager.validateAccessToken(tokenPair.accessToken);
    ...
    it('should validate valid refresh token', async () => {
      const tokenPair = await sessionManager.generateTokenPair(mockSessionData);
      const validation = await sessionManager.validateRefreshToken(tokenPair.refreshToken);
    ...
    it('should reject invalid access token', async () => {
      const validation = await sessionManager.validateAccessToken('invalid-token');
    ...
    it('should reject invalid refresh token', async () => {
      const validation = await sessionManager.validateRefreshToken('invalid-token');
  ...
  describe('Token Payload Extraction', () => {
    it('should extract access token payload correctly', async () => {
      const tokenPair = await sessionManager.generateTokenPair(mockSessionData);
      const payload = await sessionManager.extractAccessTokenPayload(tokenPair.accessToken);
      ...
      expect(payload?.deviceFingerprint).toBe(mockSessionData.deviceInfo.fingerprint);
    ...
    it('should extract refresh token payload correctly', async () => {
      const tokenPair = await sessionManager.generateTokenPair(mockSessionData);
      const payload = await sessionManager.extractRefreshTokenPayload(tokenPair.refreshToken);
    ...
    it('should return null for invalid tokens', async () => {
      const accessPayload = await sessionManager.extractAccessTokenPayload('invalid-token');
      const refreshPayload = await sessionManager.extractRefreshTokenPayload('invalid-token');
  ...
  describe('Token Blacklisting', () => {
    it('should blacklist token pair successfully', async () => {
      ...
      const accessValidation = await sessionManager.validateAccessToken(tokenPair.accessToken);
      const refreshValidation = await sessionManager.validateRefreshToken(tokenPair.refreshToken);
  ...
  describe('Session Management', () => {
    it('should create session successfully', async () => {
      ...
      expect(session.deviceFingerprint).toBe(mockSessionData.deviceInfo.fingerprint);
    ...
    it('should retrieve session by ID', async () => {
      const createdSession = await sessionManager.createSession(mockSessionData);
    ...
    it('should get user sessions', async () => {
      ...
      await sessionManager.createSession({
        ...mockSessionData,
        deviceInfo: { ...mockSessionData.deviceInfo, fingerprint: 'different-device' }
      });

      const sessions = await sessionManager.getUserSessions(mockSessionData.firebaseUid);

      expect(sessions).toHaveLength(2);
      expect(sessions.every(s => s.firebaseUid === mockSessionData.firebaseUid)).toBe(true);
    });
    ...
    it('should terminate all user sessions', async () => {
      ...
      const remainingSessions = await sessionManager.getUserSessions(mockSessionData.firebaseUid);
  ...
  describe('Device Tracking', () => {
    it('should track device information', async () => {
      const trackedDevice = await sessionManager.trackDevice(mockSessionData.deviceInfo);

      expect(trackedDevice).toBeDefined();
      expect(trackedDevice.deviceFingerprint).toBe(mockSessionData.deviceInfo.fingerprint);
  ...
  describe('Cleanup Operations', () => {
    it('should cleanup expired blacklisted tokens', async () => {
      ...
      const cleanedCount = await sessionManager.cleanupExpiredBlacklistedTokens();
  ...
  describe('Error Handling', () => {
    it('should handle invalid session data gracefully', async () => {
      ...
      await expect(sessionManager.generateTokenPair(invalidSessionData))
        .rejects.toThrow();
    });
