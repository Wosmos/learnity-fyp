...
export function getAccessibleRoutes(userRole: UserRole): string[] {
  return ROUTE_PROTECTION_RULES.filter(rule =>
...
export function getRestrictedRoutes(userRole: UserRole): string[] {
  return ROUTE_PROTECTION_RULES.filter(
    rule => !rule.allowedRoles.includes(userRole)
  ).map(rule => rule.path);
}
...
export function validateCrossRoleAccessPrevention(): {
  ...
  for (const rule of ROUTE_PROTECTION_RULES) {
    ...
    const unauthorizedRoles = allRoles.filter(
      role => !rule.allowedRoles.includes(role)
    );
...
export function performSecurityAudit(): {
  ...
  for (const rule of ROUTE_PROTECTION_RULES) {
    if (rule.allowedRoles.length > 2) {
      issues.push(
        `Route ${rule.path} allows too many roles: ${rule.allowedRoles.join(', ')}`
      );
      score -= 10;
    }
  }
  ...
  if (issues.length > 0) {
    recommendations.push(
      'Implement comprehensive route protection for all critical paths'
    );
    recommendations.push(
      'Add proper redirect rules for all unauthorized role combinations'
    );
    recommendations.push(
      'Consider implementing server-side middleware for additional security'
    );
